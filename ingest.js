...
db.connect().then(response => {
    ...
    //  Check if users already exist in the creators collection
    return checkAndInsertNewCreators(res);
}).then(() => {
...
const getOutdatedCreators = () => {
    ...
    return db.find(collections.creators, { updated: { $lt: threshold }, deleted: { $exists: false } }, false, true); // last parameter throttles the returns
}

const checkAndInsertNewCreators = (levels) => {
    const uniques = levels.filter((level, index, self) => self.map(obj => obj.name).indexOf(level.name) === index);
    const queries = uniques.map(level => {
        return db.find(collections.creators, { id: level.user }).then(arr => {
            if (!arr.length) {
                ...
                return db.insertOne(collections.queue, { creator: level.username });
            }
        });
    });
...
const getOutdatedLevels = () => {
    ...
    return db.find(collections.levels, { 
    	deleted: { $exists: false },
    	updated: { $lt: new Date( new Date().getTime() - config.threshold ) } 
    });
};
...
const getCreatorsFromQueue = () => {
    ...
    return db.distinct(collections.queue, 'creator', { creator: { $exists: true } });
};

const queryForCreatorInfo = (creators) => {
    console.log('Querying the Mega Man Maker API for creators');
    if (!creators.length) return;
    let requests = creators.map(creator => {
        // Users may be able to submit either an ID or a name which requires one of two API calls
        if (typeof creator === 'string') {
            return 'https://megamanmaker.com/megamaker/user/name?name=' + creator;
        }
        if (typeof creator === 'number') {
            return 'https://megamanmaker.com/megamaker/user/' + creator;
        }
    });
...
const updateAndAddCreatorsToDatabase = (creators) => {
    return new Promise((resolve, reject) => {
        ...
        let creatorNames = [];
        let newCreators = [];
        let deletedCreators = creators.filter(creator => creator.error);
        ...
        deletedCreators = deletedCreators.map(creator => {
            console.log(creator);
            return db.updateOne(collections.creators, { name: creator.id }, { $set: {deleted: true }});
        });

        return Promise.all(deletedCreators).then(() => {
            return db.find(collections.creators, { $or: creatorNames });
        }).then(res => {
            newCreators = creators.filter(val => {
                let match = true; // true === new insert
                for (let i of res) {
                    if (i.id === val.id) match = false; // found matching id's, it's an update 
                }
                return match;
            });
...
const getRemainingLevelsInQueue = () => {
    return db.distinct(collections.queue, 'level', { level: { $exists: true } });
};
...
const queryForLevelInfo = (levels) => {
    ...
    let requests = levels.map(level => 'https://megamanmaker.com/megamaker/level/info/' + level);
...
const insertOrUpdateRemainingLevels = (levels) => {
    ...
    let updates = levels.map(level => {
        if (level.error) {
            ...
            return db.updateOne(collections.levels, { id: level.id }, { $set: { deleted: true, updated: new Date() } });
        } else {
